...
import LikeLion.UnderTheCBackend.Service.TokenServiceForSeller;
...
@RestController
@RequestMapping("/")
public class TokenControllerForSeller {

    private final TokenServiceForSeller tokenServiceForSeller;

    @PostMapping("/api/vi/token/seller")
    @Operation(summary = "토큰", description = "토큰API", responses = {
            @ApiResponse(responseCode = "200", description = "야호! 성공!!!")
    })
    public ResponseEntity<CreateAccessTokenResponse> createNewAccessToken(@RequestBody CreateAccessTokenRequest request) {
        String newAccessToken = tokenServiceForSeller.createNewAccessToken(request.getRefreshToken());
